...
public class MechanicsHelpCommand extends CommandBase {
	...
	public void processCommand(ICommandSender sender, String[] args) {
		if (args.length == 0) { // executes /mechanics do
		...
		if (args.length == 1) {
			...
			if (args[0].equals("credits")) {
				...
				SendLineToPlayer(sender, "Text author: "
						+ LocaleDataTable.translator);
				return;
			}
			if (args[0].equalsIgnoreCase("whatamilooking") || args[0].equalsIgnoreCase("look")) {
				if (sender instanceof EntityPlayer) {
					...
					if (mop.typeOfHit.ordinal() == 0) {
						...
						SendLineToPlayer(sender, String.format("BLOCK:%s; X:%d; Y:%d; Z:%d; META:%d; SIDE:%d; NAME:%s", b.toString(), x, y, z, meta, mop.sideHit, name));
					}
